...
enum class QueryKind(
    ...
    GRADE(
        ...
        //
        // 🔴 不能收「考级课程」—— 「社会考试报名」页（/xsdjks/xsdjks_list）的表头里有
        //    「考级课程名称」，是它的子串，会把报名页当成成绩页返回（字段是
        //    报名金额 / 缴费状态 / 审核状态）。2026-09-25 加单元测试时用真机夹具发现。
        //    抓取时虽会先按 iframe 的 src 锁定目标页，但关键词是第二道保险，
        //    保险本身不能是漏的。
        // 也不能用「准考证」—— 考试安排表里就有「准考证号」，会串台。
        // 保留下来的两个词（等级类成绩 / 分数类成绩）只有真正的等级考试成绩表才有：
        // 它们由「分数类成绩」「等级类成绩」两个父列展开而来，别的表不会这么写。
        headerKeywords = listOf("等级类成绩", "分数类成绩"),
    ),
    ;
