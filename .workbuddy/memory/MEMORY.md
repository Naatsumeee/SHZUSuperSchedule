...
## 真机调试的铁律（2026-09-21 血泪）
- **先 `adb devices` 看序列号**。本会话中途设备被换过（`b18b0725` → `e87a3fe4`／2410DPN6CC 1440x3200），
  两台设备上的 APK 版本、登录态、存档**完全不同**，把两边的日志混着推理会得出完全错误的根因。
- **MIUI 下 `adb install -r` 不保证立刻替换 APK**。装完必须核对：
  `pm path` 得到的 `base.apk` 大小 == 本地 `app-release.apk` 大小（当前 14,992,995）。
  不一致就是没换上，别急着跑验证。
- 查日志先把缓冲区调大：`adb logcat -G 16M`。默认 2 MiB 会被系统噪声冲掉。
- 教务会话极脆：**连续快速请求会被判定爬虫并踢掉会话**
  （实测第 1 个请求正常、第 2 个起全部返回 164 字节空壳页）。
  `JwglQueryFetcher.STEP_DELAY_MS = 1200` 不是可选项。

## 查询存档的两个关键语义
- `QueryStore.merge` 是**按 (kind, semester) 覆盖式**合并：不在 `incoming` 里的项**保留旧值**。
  → 任何「现在没数据」的结论都必须**带一张 0 行的表**写回去，否则旧数据永远清不掉
  （曾经表现为「考试安排 · 共 6 条」这种假数据）。
- 落盘必须**原子**（`.tmp` + `renameTo`）。批量抓取要跑 2~3 分钟，
  中途进程被回收而直接 `writeText` → 半截 JSON → 下次加载整个存档归零。

## 强智教务表头
- **表头可能是多行的**，等级考试成绩表就是：第一行 6 列、数据行 11 格。
  必须按列号铺开全部 thead 行（处理 `colspan`/`rowspan`），否则值从第 7 列起全错位。
- 表里可能混入一行跨列写着「未查询到数据」的**占位行**（非空单元格 ≤1），必须过滤。
