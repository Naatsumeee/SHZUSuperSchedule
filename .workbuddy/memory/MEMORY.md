...
## 暂缓事项（有明确理由，不要当待办重提）
- **GitHub Actions 出包（CI）**：2026-09-25 评估过，方案本身可行（能绕开本机
  内存常 < 4GB、构建被 OOM 杀的痛点，且附件命名在 CI 里可控），**但用户决定暂挂**：
  本机到 GitHub 的连接本就不稳（见「网络环境」一节，代理没开时 git 与 API 全断），
  CI 会把「构建失败」和「连不上 GitHub」两种故障混在一起，反而更难排查。
  → **等开发中真的再被内存问题卡住时再上**，不是现在。
  真要动时的第一个前置检查：`compileSdk 37` 是否已正式发布
  （当前是本机复制 android-36 伪造的，CI 上没有这一步就跑不起来）。
- 其余已完成的评估项见当日日志：方案 1（单元测试）、3（版本号单一来源）、
  4（APK 移出仓库）、5（拆 SettingsPage）均已于 2026-09-25 落地。
